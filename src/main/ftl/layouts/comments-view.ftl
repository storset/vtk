...
<div class="vrtx-comments" id="comments">
  ...
  <#if comments?exists>
    ...
    <#if baseCommentURL?exists>
      <a href="${(baseCommentURL + '#comments')?html}">${header}</a>
    <#else>
      ${header}
    </#if>
  ...
  <#if postCommentURL?exists>
    <div class="add-comment" id="comment-form">
      <hr /> <@vrtx.msg code="commenting.form.add-comment" default="Add comment" />
      <form action="${postCommentURL?string?html}" method="post">
        ...
        <input type="submit" name="save" value="<@vrtx.msg code='commenting.form.submit' default='Add comment' />" />
      </form>
    </div>
  </#if>
