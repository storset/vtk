...
// USIT modified: inlineId/inlineClass to duplicate instead of remove and append back
...
function tb_show(caption, url, imageGroup) { //function called when the user clicks on a thickbox link
  try {
    ...
    // Small hack to override the "show as image" function
    var showAsHtml = url.indexOf("showAsHtml=true") !== -1;
    if (showAsHtml) {
      urlType = "";
    }
...
function tb_position() {
  $("#TB_window").css({
    marginLeft: '-' + (parseInt(TB_WIDTH, 10) / 2) + 'px',
    width: TB_WIDTH + 'px'
  });
  if (!(jQuery.browser.msie && jQuery.browser.version < 7)) { // take away IE6
    $("#TB_window").css({
      marginTop: '-' + ((parseInt(TB_HEIGHT, 10) / 2) + 30) + 'px'
    });
  }
}
