  ...
  <div id="vrtx-resource-visit">
    <div id="vrtx-resource-visit-chart">
      <img id="vrtx-resource-visit-chart-image" width="${width}" height="225" alt="Visit chart"
           ...
           title='<@vrtx.msg code="resource.metadata.about.visit.short-title" />: <#assign thisMonth = thisMonthBak><#list 0..ursNMonths as i>${months[thisMonth]}: ${ursMonths[thisMonth]?string("0")}<#if i != ursNMonths>, </#if><#if thisMonth != 0><#assign thisMonth = thisMonth - 1><#else><#assign thisMonth = 11></#if></#list>' />
    </div>
