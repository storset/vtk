...
vrtxAdmin.retrieveHTMLTemplates(fileName,
...
var templates = vrtxAdmin.retrieveHTMLTemplates("templates",
                                                ["string", "html", "radio", "dropdown", "date", "browse", "add-remove-move"],
                                                templatesRetrieved);

To make HTML with the array you have to call mustache with the desired template-name as first parameter and
a JSON-object as the second parameter. The properties of the JSON-object can be outputted with {{ }} around
...
var html = $.mustache(templates["add-remove-move"], {});
