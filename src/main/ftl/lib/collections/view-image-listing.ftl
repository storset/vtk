        ...
        <#else>
          <li class="vrtx-image-entry">
            ...
            <div class="vrtx-image-container">
                <a href="${image.URI?html}"><img src="${vrtx.linkConstructor(image.URI.toString(), 'displayThumbnailService').getPathRepresentation()}" title="${title}" alt="${title}" /></a>
            </div>
    ...
    <#list images as image>
        ...
        <#if activeImage != "">
	      <#if (activeImage == image.URI) >
	          <a href="${image.URI}" class="active"><img src="${vrtx.linkConstructor(image.URI.toString(), 'displayThumbnailService').getPathRepresentation()}" alt="${description}" title="${title}" />
	        <#else>
	          <a href="${image.URI}"><img src="${vrtx.linkConstructor(image.URI.toString(), 'displayThumbnailService').getPathRepresentation()}" alt="${description}" title="${title}" />
	        </#if>
	      <#else>
	        <#if (image_index == 0) >
	          <a href="${image.URI}" class="active"><img src="${vrtx.linkConstructor(image.URI.toString(), 'displayThumbnailService').getPathRepresentation()}" alt="${description}" title="${title}" />
    ...
    <div class="vrtx-image-table">
      <table class="rowstyle-alt colstyle-alt no-arrow" cellpadding="5" border="1">
        ...
        <#list images as image>
          <tr>
            <#local title = vrtx.propValue(image, 'title')?html />
            <td class="vrtx-table-image"><a href="${image.URI}"><img src="${vrtx.linkConstructor(image.URI.toString(), 'displayThumbnailService').getPathRepresentation()}" alt="${title}" /></a></td>
