...
<form name="vrtx-delete-resource" id="vrtx-delete-resource" action="${url}" method="post">
    ...
    <button tabindex="2" type="submit" value="cancel" id="confirmDeleteCancelAction" name="confirmDeleteCancelAction">${vrtx.getMsg("confirm-delete.cancel")}</button>
</form>
...
<script language="javascript">
	...
	$(document).ready(function(){
		setTimeout("focus();",0);
		$("#confirmDeleteCancelAction").remove(); 
		$("#vrtx-delete-resource").append('<button tabindex="2" type="button" onclick="tb_remove();" id="confirmDeleteCancelAction" name="confirmDeleteCancelAction">${vrtx.getMsg("confirm-delete.cancel")}</button>');
	});
</script>
</body>
</html>
